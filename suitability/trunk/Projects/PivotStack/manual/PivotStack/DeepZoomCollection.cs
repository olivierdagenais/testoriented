...
namespace PivotStack
{
    ...
    public class DeepZoomCollection
    {
        ...
        public DeepZoomCollection(Settings settings)
        {
            ...
            _sizeNode = CreateSizeNode(settings.ItemImageSize.Width, settings.ItemImageSize.Height);
        }

        internal static XElement CreateSizeNode(int width, int height)
        {
            #region <Size Width="800" Height="400" />
            var sizeNode = new XElement (SizeNodeName);
            sizeNode.SetAttributeValue ("Width", width);
            sizeNode.SetAttributeValue ("Height", height);
            #endregion
            return sizeNode;
        }
        ...
        {
            return CreateImageCollectionItemNode(mortonNumber, id, _settings.FileNameIdFormat, relativePathToRoot);
        }

        internal static XElement CreateImageCollectionItemNode
            (int mortonNumber, int id, string fileNameIdFormat, string relativePathToRoot)
        {
            ...
            var relativeDziSubPath = Post.ComputeBinnedPath (id, "dzi", fileNameIdFormat);
        ...
        internal IEnumerable<Bitmap> OpenLevelImages(IEnumerable<int> postIds, int level)
        {
            ...
            var absoluteOutputFolder = _settings.AbsoluteOutputFolder;
            ...
            foreach (var postId in postIds)
            {
                var absoluteSourceImagePath = FormatAbsoluteSourceImagePath(postId, fileNameIdFormat, levelName, 
                    inputFileName, absoluteOutputFolder);
        ...
        internal static string FormatAbsoluteSourceImagePath
            (int postId, string fileNameIdFormat, string levelName, string inputFileName, string absoluteOutputFolder)
        {
            ...
            var result = Path.Combine (absoluteOutputFolder, relativeLevelFolder);
            return result;
        }
